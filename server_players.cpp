...
const int gm_to_pl[4] = {eventfd(0, EFD_SEMAPHORE), eventfd(0, EFD_SEMAPHORE),
...
void send_DEAL(SendData &send_data, const std::vector<Card> &hand) {
    std::string s = "DEAL" + std::to_string(game.get_deal()) +
        game.get_player() + cards_to_string(hand) + "\r\n";
    if (writen(send_data, s.c_str(), s.size()) < static_cast<ssize_t>(s.size()))
...
void process_hand (SendData &send_data, const int &pos, std::vector <Card> &hand) {
    ...
    pollfd fds[2];
    fds[0] = {.fd = gm_to_pl[pos], .events = POLLIN, .revents = 0};
    ...
    decrement_event_fd(gm_to_pl[pos]);
...
void wait_for_turn (int pos, int client_fd) {
    ...
    while (true) {
        ...
        if (!game_paused)
            ...
            decrement_event_fd(gm_to_pl[pos]); // wait for unpause
    }
}
    ...
    try {
        ...
        while (trick_no <= 13) {
            ...
            if (trick.size() == 4)
                ...
                increment_event_fd(gm_to_pl[(pos + 1) % 4]); // TODO: rename to to_pl?
    ...
    while (get_deal(desc, current_deal, starting_client)) {
        ...
        while (tricks < 13) {
            ...
            if (active.is_four()) {
                ...
                game_paused = true;
                for (int i = 0; i < 4; i++) {
                    if (active.is_active(i))
                        increment_event_fd(gm_to_pl[i]);
                }
                active.wait_for_four();
                game_paused = false;
                for (const auto &fd: gm_to_pl)
